...
def trainer_train(
    ...
    print("trainer.sft_args",trainer.sft_args)
...
def llm_sft_old(args: SftArguments) -> Dict[str, Any]:
    ...
    # === DEBUG START: 打印训练层信息 ===
    print("\n" + "="*30 + " TRAINABLE LAYERS CHECK (ORIGINAL) " + "="*30)
    trainable_layers = []
    total_params = 0
    ...
    print("-" * 60)
    print(f"[Summary] Total Trainable Tensors: {len(trainable_layers)}")
    print(f"[Summary] Total Trainable Parameters: {total_params}")
    
    # 检查关键模块是否存在 (针对 Qwen2-VL)
    has_vision = any("visual" in n for n in trainable_layers)
    has_mlp = any("mlp" in n for n in trainable_layers)
    has_attn = any("self_attn" in n or "attn" in n for n in trainable_layers)
    
    print(f"[Check] Includes Visual Encoder layers? {has_vision}")
    print(f"[Check] Includes MLP layers? {has_mlp}")
    print(f"[Check] Includes Attention layers? {has_attn}")
    print("="*80 + "\n")
    # === DEBUG END ===
...
def llm_sft(args: SftArguments) -> None:
    ...
    for i in range(args.round):
        ...
        if args.fed_alg == 'fedadam':
            ...
            fedopt_beta1 = 0.9
            fedopt_beta2 = 0.999
            fedopt_eta = 1e-3  # 降低学习率
            fedopt_tau = 1e-8 

            for key in global_lora_new:
                delta_w = global_lora_new[key] - previous_global_lora[key]
                # 一阶动量
                proxy_dict[key] = fedopt_beta1 * proxy_dict[key] + (1 - fedopt_beta1) * delta_w 
                # 二阶动量
                opt_proxy_dict[key] = fedopt_beta2 * opt_proxy_dict[key] + (1 - fedopt_beta2) * torch.square(delta_w)  
                # 偏差修正
                m_hat = proxy_dict[key] / (1 - fedopt_beta1 ** (i+1))
                v_hat = opt_proxy_dict[key] / (1 - fedopt_beta2 ** (i+1))

                global_lora[key] += fedopt_eta * m_hat / (torch.sqrt(v_hat) + fedopt_tau)
            ...
            for key in global_lora_new:
                ...
                opt_proxy_dict[key] = opt_proxy_dict[key] - (1 - fedopt_beta2) * delta_square * torch.sign(
                    opt_proxy_dict[key] - delta_square)       
